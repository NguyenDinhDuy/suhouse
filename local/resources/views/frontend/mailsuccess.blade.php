...
<div style="border: 1px solid #ccc;background: #eee;margin: 50px 15px;padding: 10px 20px" class="container">
    <div>
        <p>Thân gửi bạn {{$order->bill_name}},</p>
        <p>Cảm ơn bạn đã mua sắm tại website SUHOUSE! </p>
        <p>Đơn hàng <span style="color: #E32124;font-weight: bold">#{{$order->bill_code}}</span> (Thời gian đặt
            hàng {{ $order->created_at }}) của bạn hiện đã được tiếp nhận
            và đang
    ...
    <div>
        <div>
            ...
            <div style="border:1px solid #555;background-color: #fff;padding: 5px 20px">
                <p>Tên: {{$order->bill_name}}</p>
                <p>Số điện thoại: {{$order->bill_phone}}</p>
                <p>Địa chỉ: {{$order->address}}</p>
                <p>Hình thức thanh toán: {{$order->bill_payment }}</p>
            </div>
        ...
        <div style="margin-top: 10px">
            ...
            <table style="border: 1px solid #555;padding:10px 20px;background: #fff" class="table table-bordered">
                ...
                <tr>
                    ...
                    <th style="border: 1px solid #555">Thông tin</th>
                ...
                @foreach($order->products as $product)
                    ...
                    <tr>
                        {{--<td>--}}
                        {{--<img src="{{asset('/img/'.$options['img'])}}">--}}
                        {{--Hình ảnh:--}}
                        {{--<img src="{{ $message->embed(asset("/local/storagepp/avatar/".$options['img'])) }}">--}}
                        {{--<img src="{{ 'Here'->embed(asset("/local/storagepp/avatar/".$options['img'])) }}">--}}
                        {{--src="{{ asset("/local/storagepp/avatar/".$options['img'])}}">--}}
                        {{--</td>--}}
                        ...
                        <td style="border: 1px solid #ccc"><span>{{ $product->pivot->qty }}</span></td>
                        <td style="border: 1px solid #ccc">
